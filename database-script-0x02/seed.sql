...
VALUES
    (UUID(), 'Alice', 'Johnson', 'alice@example.com', 'hashed_password_1', '1234567890', 'guest', DEFAULT),
    (UUID(), 'Bob', 'Smith', 'bob@example.com', 'hashed_password_2', '0987654321', 'host', DEFAULT),
    (UUID(), 'Charlie', 'Brown', 'charlie@example.com', 'hashed_password_3', '1112223333', 'admin', DEFAULT),
    (UUID(), 'Diana', 'Prince', 'diana@example.com', 'hashed_password_4', '4445556666', 'guest', DEFAULT),
    (UUID(), 'Ethan', 'Hunt', 'ethan@example.com', 'hashed_password_5', '7778889999', 'host', DEFAULT);
...
VALUES
    (UUID(), (SELECT user_id FROM User WHERE email = 'bob@example.com'), 'Cozy Apartment', 'A cozy apartment in downtown.', 'New York', 120.00, DEFAULT, DEFAULT),
    (UUID(), (SELECT user_id FROM User WHERE email = 'bob@example.com'), 'Beach House', 'Beautiful beach house with ocean view.', 'San Francisco', 300.00, DEFAULT, DEFAULT),
    (UUID(), (SELECT user_id FROM User WHERE email = 'ethan@example.com'), 'Luxury Villa', 'Exclusive villa with modern amenities.', 'Los Angeles', 500.00, DEFAULT, DEFAULT),
    (UUID(), (SELECT user_id FROM User WHERE email = 'ethan@example.com'), 'Country Cottage', 'Charming cottage in the countryside.', 'Austin', 200.00, DEFAULT, DEFAULT),
    (UUID(), (SELECT user_id FROM User WHERE email = 'bob@example.com'), 'Penthouse Suite', 'High-rise suite with city views.', 'Chicago', 350.00, DEFAULT, DEFAULT);
